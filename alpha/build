...
_G.ncplist = { 'Soldier', 'Smoky', 'Tashi', 'Clown', 'Clown Pirate', 'The Clown', 'Marine', 'Dark Leg', 'Weapon Man',
               'Trainer Chef', 'Commander', 'Captain', 'Axe', 'SharkMan', 'True Karate Fishman', 'Karate Fishman',
               'Fishman', 'Quake Woman', 'New World Pirate', 'Rear Admiral', 'Leader', 'Pasta', 'Leo', 'Wolf',
               'Giraffe', 'Shadow Master', 'Bomb Man', 'Zombie', 'Oars', 'Rumble Man', 'Ball Man', 'Sky Soldier',
               'King of Sand', 'Candle Man', 'Heavy Man', 'King Snow', 'Snow Soldier', 'Shanks', 'Seasoned Fishman',
               'Sword Fishman', 'Combat Fishman', 'Fishman Soldier' }

_G.islands = { 'Stone Rain Sea', 'Town', 'Pirate Island', 'Soldier Town', 'Chef Ship', 'Snow Island', 'Desert Island',
               'Skyland', 'Bubbleland', 'Lobby Island', 'War Island', 'Fishland', 'Stone Arena', 'Zombie Island',
               'Shark Island', 'Board' }
-- functions
...
utility.tween = function(time, pos) -- Regular tween
    game:GetService('TweenService'):Create(HumanoidRootPart, TweenInfo.new(time, Enum.EasingStyle.Quart), {
...
utility.bounce = function(time, pos) -- tiny tween
    game:GetService('TweenService'):Create(HumanoidRootPart, TweenInfo.new(time, Enum.EasingStyle.Bounce), {
        CFrame = pos + Vector3.new(0, 2, 0)
     }):Play()
    task.wait(time)
end
...
utility.farm = function(mob)
    for i, v in pairs(standardNcpFolder:GetDescendants()) do
        if v:FindFirstChild('Humanoid') and v.HumanoidRootPart and v.Name == mob and v.Humanoid.Health > 0 and
            (v.HumanoidRootPart.Position - game:GetService('Players').LocalPlayer.Character.HumanoidRootPart.Position).magnitude <=
            2000 then
            repeat
                task.wait(.1)
                utility.useSword()
...
utility.huntplayer = function(mob)
    for i, v in pairs(game.Workspace:GetDescendants()) do
        if v:FindFirstChild('Humanoid') and v.HumanoidRootPart and v.Name == _G.selectedplayer and v.Humanoid.Health > 0 and
            (v.HumanoidRootPart.Position - game:GetService('Players').LocalPlayer.Character.HumanoidRootPart.Position).magnitude <=
            2000 then
            repeat
                task.wait(.1)
    ...
    for _, v in next, Player.Backpack:GetChildren() do
        if v:IsA('Tool') and v.Name == fruit then
            for i, c in pairs(game.Workspace:GetDescendants()) do
                if c:FindFirstChild('Humanoid') and c.HumanoidRootPart and c.Name == _G.selectedplayer and
                    c.Humanoid.Health > 0 and
                    (c.HumanoidRootPart.Position -
                        game:GetService('Players').LocalPlayer.Character.HumanoidRootPart.Position).magnitude <= 2000 then
...
local versionpicker = versionsec:addDropdown('Versions', { 'Dev-Testing', 'Beta-Testing', 'Alpha-Testing' },
    function(text)
        wantedversion = text
    end)
...
local farm_method = farm:addDropdown('Farm Method' or _G.farmtpmethod, { 'Tween', 'Warp', 'Bounce', 'Standard' },
    function(text)
